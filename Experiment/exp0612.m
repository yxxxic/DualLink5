...
data_dir = fullfile(pwd, '实验数据', '0612');
...
if numel(angle_files) ~= 6
    warning('Expected 6 paired experiments, found %d pairs.', numel(angle_files));
end

fprintf('Found %d paired 0612 experiment files.\n', numel(angle_files));
...
for i = 1:numel(angle_files)
    ...
    fig = plotComparison( ...
        i, shape_name, angle_files(i), record_files(i), ...
        optical, x_g_mt, y_g_mt, x_g_rs, y_g_rs);

    output_name = sprintf('exp0612_%02d_%s.png', i, shape_name);
    figure(fig);
    saveFigIEEE(fullfile(output_dir, erase(output_name, '.png')));

    fprintf('%02d %-6s: %s  <->  %s  (mt spikes: angle1=%d, angle2=%d, invalid=%d, jumps=%d, discarded=%d)\n', ...
        i, shape_name, angle_files(i), record_files(i), ...
...
function optical = readOpticalRecord(file)
    ...
    optical.x = T.("aimooe_coord_position.x") - 40;
...
function fig = plotComparison( ...
    group_id, shape_name, angle_name, record_name, ...
    optical, x_g_mt, y_g_mt, x_g_rs, y_g_rs)

    fig = figure('Color', 'w', ...
        'Name', sprintf('exp0612 %02d %s', group_id, shape_name));
    ax = axes(fig);
    ...
    xlabel('x / mm');
    ylabel('y / mm');
    title(sprintf('exp0612 %02d %s: %s <-> %s', ...
        group_id, shape_name, angle_name, record_name), ...
        'Interpreter', 'none');
